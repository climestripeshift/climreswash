...
def generate_brief(district_name: str, ranking: dict, gap_data: dict | None, out_path: Path):
    ...
    # Future projection (if gap data available)
    if gap_data:
    ...
    story.append(Paragraph(
        ...
        f"WRIS (groundwater). Risk methodology: ClimResWASH formula book (IPCC AR6 framework). "
        f"Calibrated against 5 real disaster events.",
        ParagraphStyle("Footer", parent=small_style, fontSize=6.5, textColor=colors.lightgrey)))
    story.append(Paragraph(
        f"Generated by <b>ClimResWASH</b> · {datetime.now().strftime('%d %B %Y')} · "
