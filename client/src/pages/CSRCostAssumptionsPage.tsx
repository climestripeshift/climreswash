...
const FIELDS: { key: keyof UnitCosts; label: string; icon: string; hint: string }[] = [
  { key: "toilet", label: "Toilet construction", icon: "🚽", hint: "per toilet, from toilet_required_count" },
  { key: "classroomRepair", label: "Classroom major repair", icon: "🛠️", hint: "per classroom needing repair" },
...
export default function CSRCostAssumptionsPage() {
  ...
  const statewideNeed = useMemo(() => {
    const totals = { toilet_required_count: 0, classroom_repair_needed_count: 0, building_dilapidated_count: 0, new_classroom_requirement_count: 0 };
  ...
  const byCategory = FIELDS.map((f) => {
    const countKey = { toilet: "toilet_required_count", classroomRepair: "classroom_repair_needed_count",
      dilapidatedBuilding: "building_dilapidated_count", newClassroom: "new_classroom_requirement_count" }[f.key] as keyof typeof statewideNeed;
      ...
      <div className="flex-1 overflow-y-auto p-4">
        <div className="max-w-2xl mx-auto space-y-4">
          <div className="px-4 py-2.5 rounded-lg border border-border/30 bg-amber-500/5 text-[11px] text-muted-foreground leading-relaxed">
            ⚠️ These are planning-level estimates you set yourself — not sourced from any official schedule of
            rates. Saved to this browser only (not shared with other visitors or synced anywhere), and used to
            compute the "💰 Funding Required" metric and column on the CSR map. Editing here never re-fetches
            that page's data.
          </div>

          <div className="border border-border/40 rounded-lg overflow-hidden">
            <div className="p-3 border-b border-border/30 bg-muted/20 text-xs font-semibold">Cost per unit (₹)</div>
          ...
          <div className="border border-border/40 rounded-lg overflow-hidden">
            <div className="p-3 border-b border-border/30 bg-muted/20">
              ...
              <div className="text-[10px] text-muted-foreground mt-0.5">Live preview — updates as you type, before you save.</div>
            </div>
            <div className="divide-y divide-border/20">
              {byCategory.map((f) => (
