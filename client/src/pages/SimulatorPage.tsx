...
const PRESETS = [
  ...
  },
  {
    ...
    note: "Marathwada 2015 — 11 mm/day (severe deficit) · hits normally-wetter districts hardest, not the driest",
...
export default function SimulatorPage() {
  ...
  // State-level AC fallback for null-AC hexes (see chat, simulator null-AC fix) --
  // same numbers hex_risk.py's state_ac uses, generated by build_wash_state_context().
  const stateAcQ = useQuery<Record<string, number>>({
    queryKey: ["state-ac-sim"],
    queryFn: () => fetch("/data/state_ac.json").then(r => r.json()),
    staleTime: Infinity,
  });
  ...
  const simScores = useMemo(() => {
    ...
    const stateAc = stateAcQ.data;
    if (!hexes || !stateAc) return new Map<string, number>();
    const m = new Map<string, number>();
    for (const hex of hexes) m.set(hex.h3_id, reScoreHex(hex, debounced, stateAc));
    return m;
  }, [hexQ.data?.props, stateAcQ.data, debounced]);
